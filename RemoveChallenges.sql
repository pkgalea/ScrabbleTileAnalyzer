...
CREATE VIEW player2scoreView as SELECT turn_id, COALESCE(lag(p2_score, 1) OVER (PARTITION BY gamenum order by movenum), 0) as prevscore, p2_score from turn where is_player2 = ' 1';
UPDATE turn SET turn_score = t.p2_score - t.prevscore FROM player2scoreView t where turn.turn_id = t.turn_id

DROP VIEW player1scoreView;
CREATE VIEW player1scoreView as SELECT turn_id, COALESCE(lag(p1_score, 1) OVER (PARTITION BY gamenum order by movenum), 0) as prevscore, p1_score from turn where is_player2 = ' 0';
UPDATE turn SET turn_score = t.p1_score - t.prevscore FROM player1scoreView t where turn.turn_id = t.turn_id
